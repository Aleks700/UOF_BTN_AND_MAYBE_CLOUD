...
function createTableForKmlKmz(coordinates) {
  ...
  const selectedSatellites = Array.from(
    document.querySelectorAll('input[name="satellite"]:checked')
...
function getCoordinatesFromKML(kml) {
  ...
  for (let i = 0; i < placemarks.length; i++) {
    ...
    for (let j = 0; j < multiGeometries.length; j++) {
      ...
      for (let k = 0; k < polygons.length; k++) {
        const coordinatesNode =
          polygons[k].getElementsByTagName("coordinates")[0];
    ...
    for (let m = 0; m < polygonsDirect.length; m++) {
      const coordinatesNode =
        polygonsDirect[m].getElementsByTagName("coordinates")[0];
      processCoordinatesNode(coordinatesNode, coordinates);
    }
  }
...
document
  .getElementById("headerBtnKmlUpload")
...
function openInfoBox(imageData) {
  ...
  if (!imageData) {
    ...
    content +=
      "<p><strong>Imagery date:</strong> " + imageData.Meta_Date + "</p>";
    ...
    if (imageData.IncidenceAngle !== null) {
      ...
      content +=
        "<p><strong>Incidence angle:</strong> " +
        imageData.IncidenceAngle +
        "</p>";
      content +=
        "<p><strong>Incidence angle:</strong> " + imageData.Cloud + "</p>";
    }
...
document.getElementById("overlay").addEventListener("click", function (event) {
  ...
  var isClickInsideInfoBox = document
